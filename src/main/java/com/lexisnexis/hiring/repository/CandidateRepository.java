...
public interface CandidateRepository extends JpaRepository<Candidate, Integer> {
    ...
    @Query(value = "SELECT * FROM candidate_table " +
            " WHERE requisition_name_job_id IN (SELECT job_id FROM requisition_table WHERE manager_employee_id = ?1)  AND selection_date IS NOT NULL AND result = 'L1Scheduled'  OR result = 'L1ReScheduled' ", nativeQuery = true)
    List<Candidate> getLevelOneCandidates(Integer managerId);

    @Query(value = "SELECT * FROM candidate_table " +
            " WHERE requisition_name_job_id IN (SELECT job_id FROM requisition_table WHERE manager_employee_id = ?1)  AND level1date IS NOT NULL AND result = 'L2Scheduled'  or result = 'L2ReScheduled' ", nativeQuery = true)
