...
public class RequisitionServiceImpl implements RequisitionService {
	...
	public Requisition addRequisition(Requisition requisition, int managerID) throws InvalidEmployeeID {
		Employee findByemployeeId = employeeRepository.findByemployeeId(managerID);
		if(findByemployeeId!=null) {
			Employee findByemployeeId2 = employeeRepository.findByemployeeId(requisition.getSpecificPanel().getEmployeeId());
			requisition.setManager(findByemployeeId);
			requisition.setSpecificPanel(findByemployeeId2);
			Date currentDate = new Date();
			requisition.setCreatedDate(currentDate);
			requisition.setUpdatedDate(currentDate);
	...
	public Requisition updateRequisition(Requisition requisition,int jobId) throws InvalidEmployeeID {
		...
		if(findByjobId!=null) {
			...
			Date currentDate = new Date();
			findByjobId.setUpdatedDate(currentDate);
	...
	public List<Requisition> getAllRequisition(int managerId) throws InvalidEmployeeID{
		List<Requisition> findByManager = requisitionRepository.findByManager(employeeRepository.findByemployeeId(managerId));
